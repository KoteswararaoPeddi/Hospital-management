...
Grounded in the real MediNex+ frontend (`frontend/src`). Every entry references a file that exists.
...
1. **❌ naive vs ✅ our real code**
   ...
   <Accordion><Item q="What is MediNex+?" a="…" /><Item q="How do I onboard?" a="…" /></Accordion>
...
1. **❌ naive vs ✅ our real code**
   ...
   if (await confirm({ title: "Delete patient record?", destructive: true })) doDelete();
   ```
