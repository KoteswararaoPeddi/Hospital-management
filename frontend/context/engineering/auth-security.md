...
> `features/auth/schemas/auth.schema.ts`). The auth *server* is **not built yet** — the backend has no
> `modules/auth/` and no `jwt-auth.guard.ts`, even though `app.module.ts` imports them. So the frontend
> is coded against a backend contract (`/auth/login`, `/auth/refresh`, `/auth/me`, httpOnly cookies)
> that still has to be implemented. Entries below describe the client design and the intended server
> contract; see Gotchas.
...
- **Server side is unbuilt.** `app.module.ts` registers a global `JwtAuthGuard` and imports
  `modules/auth/auth.module`, but neither file exists in `backend/src/`. The frontend already calls
  `/auth/register|login|logout|me|refresh`. **Lesson:** the client is ahead of the server — those
  endpoints, the JWT issuing/rotation, and httpOnly cookie setting must be implemented for any of this
  auth flow to actually work. Track as `[~]`, not done.
...
- **`hashedRefreshToken` implies refresh-token rotation server-side** — the `User.hashedRefreshToken`
  column ([`schema.prisma:67`](../../../backend/prisma/schema.prisma)) stores a bcrypt hash of the
  *current* refresh token so the server can detect reuse/replay. It is not yet migrated (see
  [database.md](database.md) drift note). `(no lesson — pure fact)`
